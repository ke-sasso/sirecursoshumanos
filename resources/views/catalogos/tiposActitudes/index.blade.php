...
<a href="{{route('insertar.tipoActitud')}}" style="" type="button" id="cancelar" class="btn btn-primary m-t-10"><i class="fa fa-plus" aria-hidden="true"></i>Nuevo tipo de actitud</a>
</div>
</div>
	...
	<table class="table table-striped table-hover"  id="tr-tipoActitudes" style="font-size:14px;" width="100%">
...
<div class="modal fade modal-center" id="info"  style='display:none;' tabindex="-1" role="dialog" >
    <div class="modal-dialog modal-lg" >
      ...
      <div class="modal-body">
        ...
        <form role="form" method="post" action="{{route('post.editar.tipoActitud')}}"   autocomplete="off" id="formModal">
                  <div class="form-group">
                     <div class="row">
                    <div class="col-sm-6 col-md-6">
                     <div class="input-group ">
                      <div class="input-group-addon"><b>Nombre del tipo de actitud</b></div>
...
$( document ).ready(function() {
    ...
    var table = $('#tr-tipoActitudes').DataTable({
        filter:true,
        processing: true,
        serverSide: false,

        ajax: {
            url: "{{route('dt.row.data.tipoActitud')}}",
        },
        columns: [
	      	{ "data": null }, // <-- This is will your index column
          {data: 'nombreTipoActitud', name: 'nombreTipoActitud'},
...
function editarInfo(id){
    ...
    $.get("{{route('data.tipoActitud')}}?param="+id, function(data) {
                    try{
                     $('#nombre').val(""); 
                      $('#id').val("");
                      document.getElementById("estado").length=0;

                      $('#nombre').val(data.nombreTipoActitud); 
                      $('#id').val(data.idTipoActitud);
                      if(data.activo==1){
      ...
      success:  function (response){
            ...
            if(isJson(response)){
              alertify.alert("Mensaje de Sistema","<strong><p class='text-justify'>¡Informaci&oacute;n actualizada de forma exitosa!</p></strong>",function(){
                ...
                window.location.href = "{{route('tipoActitud.listar')}}";
              });
