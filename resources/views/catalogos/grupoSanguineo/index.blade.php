...
<a href="{{route('insertar.grupoSang')}}" style="" type="button" id="cancelar" class="btn btn-primary m-t-10"><i class="fa fa-plus" aria-hidden="true"></i>Nuevo grupo sanguineo</a>
</div>
</div>
...
<div class="modal fade modal-center" id="info"  style='display:none;' tabindex="-1" role="dialog" >
    <div class="modal-dialog modal-lg" >
      ...
      <div class="modal-body">
        ...
        <form role="form" method="post" action="{{route('post.editar.grupoSang')}}"   autocomplete="off" id="formModal">
                     <div class="form-group">
                     <div class="row">
                    <div class="col-sm-12 col-md-12">
                     <div class="input-group ">
                      <div class="input-group-addon"><b>Nombre grupo sanguineo</b></div>
                      <input type="text" class="form-control" id="nombreGrupo" name="nombreGrupo" autocomplete="off" >
                    </div>
                    </div>
                    </div>
                </div>
...
$( document ).ready(function() {
    var table = $('#tr-Grupo').DataTable({
        ...
        ajax: {
            url: "{{route('dt.row.data.grupoSang')}}",
        },
        columns: [
...
function editarInfo(id){
    ...
    $.get("{{route('data.grupoSang')}}?param="+id, function(data) {
                    try{
                     $('#nombreGrupo').val(""); 
                      $('#id').val("");
      ...
      success:  function (response){
            ...
            if(isJson(response)){
              alertify.alert("Mensaje de Sistema","<strong><p class='text-justify'>¡Informaci&oacute;n actualizada de forma exitosa!</p></strong>",function(){
                ...
                window.location.href = "{{route('grupoSang.listar')}}";
              });
