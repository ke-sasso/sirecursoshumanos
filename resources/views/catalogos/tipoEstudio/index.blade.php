...
<a href="{{route('insertar.tipoEst')}}" style="" type="button" id="cancelar" class="btn btn-primary m-t-10"><i class="fa fa-plus" aria-hidden="true"></i>Nuevo Tipo de estudio</a>
</div>
</div>
	...
	<table class="table table-striped table-hover"  id="tr-tipoEst" style="font-size:14px;" width="100%">
...
<div class="modal fade modal-center" id="info"  style='display:none;' tabindex="-1" role="dialog" >
    <div class="modal-dialog modal-lg" >
      ...
      <div class="modal-body">
        ...
        <form role="form" method="post" action="{{route('post.editar.tipoEst')}}"   autocomplete="off" id="formModal">
                  <div class="form-group">
                     <div class="row">
                    <div class="col-sm-12 col-md-12">
                     <div class="input-group ">
                      <div class="input-group-addon"><b>Nombre tipo de estudio</b></div>
...
$( document ).ready(function() {
    var table = $('#tr-tipoEst').DataTable({
        filter:true,
        processing: true,
        serverSide: false,

        ajax: {
            url: "{{route('dt.row.data.tipoEst')}}",
        },
        columns: [
...
function editarInfo(id){
    ...
    $.get("{{route('data.tipoEst')}}?param="+id, function(data) {
                    try{
                     $('#nombre').val(""); 
                      $('#id').val("");
                      $('#nombre').val(data.nombreTipo); 
                      $('#id').val(data.idTipo);
                    }
                    catch(e)
                    {
      ...
      success:  function (response){
            ...
            if(isJson(response)){
              alertify.alert("Mensaje de Sistema","<strong><p class='text-justify'>¡Informaci&oacute;n actualizada de forma exitosa!</p></strong>",function(){
                ...
                window.location.href = "{{route('tipoEst.listar')}}";
              });
