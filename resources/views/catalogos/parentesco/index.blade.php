...
<a href="{{route('insertar.parentesco')}}" style="" type="button" id="cancelar" class="btn btn-primary m-t-10"><i class="fa fa-plus" aria-hidden="true"></i>Nuevo Parentesco</a>
</div>
</div>
	...
	<table class="table table-striped table-hover"  id="tr-Parentesco" style="font-size:14px;" width="100%">
...
<div class="modal fade modal-center" id="info"  style='display:none;' tabindex="-1" role="dialog" >
    <div class="modal-dialog modal-lg" >
      ...
      <div class="modal-body">
        ...
        <form role="form" method="post" action="{{route('post.editar.parentesco')}}"   autocomplete="off" id="formModal">
...
$( document ).ready(function() {
    var table = $('#tr-Parentesco').DataTable({
        filter:true,
        processing: true,
        serverSide: false,

        ajax: {
            url: "{{route('dt.row.data.parentesco')}}",
        },
        columns: [
	      	{ "data": null }, // <-- This is will your index column
          {data: 'nombreParentesco', name: 'nombreParentesco'},
...
function editarInfo(id){
    ...
    $.get("{{route('data.parentesco')}}?param="+id, function(data) {
                    try{
                     $('#nombreParentesco').val(""); 
                      $('#id').val("");
                      $('#nombreParentesco').val(data.nombreParentesco); 
                      $('#id').val(data.idParentesco);
                    }
                    catch(e)
                    {
                      console.log(e);
      ...
      success:  function (response){
            ...
            if(isJson(response)){
              alertify.alert("Mensaje de Sistema","<strong><p class='text-justify'>¡Informaci&oacute;n actualizada de forma exitosa!</p></strong>",function(){
                ...
                window.location.href = "{{route('parentesco.listar')}}";
              });
