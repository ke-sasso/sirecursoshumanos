...
<a href="{{route('insertar.banco')}}" style="" type="button" id="cancelar" class="btn btn-primary m-t-10"><i class="fa fa-plus" aria-hidden="true"></i>Nuevo banco</a>
</div>
</div>
...
<div class="modal fade modal-center" id="info"  style='display:none;' tabindex="-1" role="dialog" >
    <div class="modal-dialog modal-lg" >
      ...
      <div class="modal-body">
        ...
        <form role="form" method="post" action="{{route('post.editar.banco')}}"   autocomplete="off" id="formModal">
                  <div class="form-group">
                     <div class="row">
                    <div class="col-sm-12 col-md-12">
                     <div class="input-group ">
                      <div class="input-group-addon"><b>Nombre del Banco</b></div>
...
$( document ).ready(function() {
    var table = $('#tr-Parentesco').DataTable({
        ...
        ajax: {
            url: "{{route('dt.row.data.banco')}}",
        },
        columns: [
	      	{ "data": null }, // <-- This is will your index column
          {data: 'nombreBanco', name: 'nombreBanco'},
...
function editarInfo(id){
    ...
    $.get("{{route('data.banco')}}?param="+id, function(data) {
                    try{
                     $('#nombre').val(""); 
                      $('#id').val("");
                      $('#nombre').val(data.nombreBanco); 
                      $('#id').val(data.idBanco);
                    }
                    catch(e)
                    {
      ...
      success:  function (response){
            ...
            if(isJson(response)){
              alertify.alert("Mensaje de Sistema","<strong><p class='text-justify'>¡Informaci&oacute;n actualizada de forma exitosa!</p></strong>",function(){
                ...
                window.location.href = "{{route('banco.listar')}}";
              });
